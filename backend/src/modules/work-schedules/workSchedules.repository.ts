...
export async function create(input: CreateWorkScheduleInput, companyId: string): Promise<WorkScheduleRow> {
  const result = await query<WorkScheduleRow>(
    `INSERT INTO work_schedules (company_id, name, weekly_hours, daily_hours, work_days, break_minutes, is_rotating)
     VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
    [companyId, input.name, input.weeklyHours || null, input.dailyHours || null,
     input.workDays || null, input.breakMinutes, input.isRotating]
  );
...
export async function update(id: string, input: Partial<CreateWorkScheduleInput>, companyId: string): Promise<WorkScheduleRow | null> {
  ...
  if (input.workDays !== undefined) { fields.push(`work_days = $${idx}`); values.push(input.workDays); idx++; }
  ...
  const result = await query<WorkScheduleRow>(`UPDATE work_schedules SET ${fields.join(', ')} WHERE id = $${idx} AND company_id = $${idx + 1} RETURNING *`, values);
  return result.rows[0] || null;
}
