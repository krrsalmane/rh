...
export async function findAll(filters: AbsenceFiltersInput, companyId: string) {
  ...
  const countResult = await query<{ count: string }>(`SELECT COUNT(*) as count FROM absences a WHERE ${whereClause}`, params);
  const total = parseInt(countResult.rows[0].count, 10);
  const offset = (filters.page - 1) * filters.limit;
  const result = await query<AbsenceRow>(
    `SELECT a.*, e.first_name || ' ' || e.last_name as employee_name FROM absences a JOIN employees e ON a.employee_id = e.id WHERE ${whereClause} ORDER BY a.start_date DESC LIMIT $${idx} OFFSET $${idx + 1}`,
...
export async function create(input: CreateAbsenceInput, companyId: string): Promise<AbsenceRow> {
  const result = await query<AbsenceRow>(
    'INSERT INTO absences (company_id, employee_id, start_date, end_date, type, reason) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *',
    [companyId, input.employeeId, input.startDate, input.endDate, input.type || null, input.reason || null]
  );
...
export async function justify(id: string, reviewNote: string, attachments: string[], reviewedBy: string, companyId: string): Promise<AbsenceRow | null> {
  const result = await query<AbsenceRow>(
    `UPDATE absences SET justification_status = 'justified', review_note = $1, attachments = $2, reviewed_by = $3 WHERE id = $4 AND company_id = $5 RETURNING *`,
...
export async function markUnjustified(id: string, reviewNote: string, reviewedBy: string, companyId: string): Promise<AbsenceRow | null> {
  const result = await query<AbsenceRow>(
    `UPDATE absences SET justification_status = 'unjustified', review_note = $1, reviewed_by = $2 WHERE id = $3 AND company_id = $4 RETURNING *`,
